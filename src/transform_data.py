...
def normalize_weather_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_weather = df_weather.rename(columns={
        'id': 'weather_id',
        'main': 'wheather_main',
        'description': 'weather_descripiton',
        'icon': 'weather_icon'
        })
...
def drop_columns(df: pd.DataFrame, columns_names: list[str]) -> pd.DataFrame:
    ...
    logging.info(f"Colunas removidas - {len(df.columns)}")   
    return df


def columns_to_rename(df: pd.DataFrame, columns_names: dict[str, str]) -> pd.DataFrame:
...
def normalize_datetime_columns(df: pd.DataFrame, columns_names: list[str]) -> pd.DataFrame:
    logging.info(f"Convertendo colunas para datetime: {columns_names}")
    
    for name in columns_names:
        df[name] = pd.to_datetime(df[name], unit='s', utc=True).dt.tz_convert('America/Sao_Paulo')
    logging.info(f"Data convertida com sucesso")
    return df


def data_transformations():
    logging.info("Iniciando a trasformação")
    df = create_dataframe(path_name)
    df = normalize_weather_columns(df)
    df = drop_columns(df, columns_names_to_drop)
    df = columns_to_rename(df, columns_names_to_rename)
